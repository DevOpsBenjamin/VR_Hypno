...
type Path<T, Prev extends string[] = []> = T extends NavigationLeaf<any>
  ? Prev
  : T extends object
...
export type OptionForPath<T> = T extends NavigationLeaf<infer O>
  ? O
  : undefined;
...
function buildNavPaths<T extends object>(
  tree: T,
  target: any = nav,
  path: string[] = []
): void {
